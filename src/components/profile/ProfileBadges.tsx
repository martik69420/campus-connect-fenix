...
const ProfileBadges: React.FC<ProfileBadgesProps> = ({ badges, className }) => {
  // Filter out earned badges to display at the top
  const earnedBadges = badges.filter(badge => badge.earned);
  const unearnedBadges = badges.filter(badge => !badge.earned);
  
  // Discord-style badges to display inline at the top
  const renderDiscordStyleBadges = () => {
    if (earnedBadges.length === 0) return null;
    
    return (
      <div className="flex flex-wrap gap-2 mb-6 p-4 bg-muted/30 rounded-lg border">
        <h3 className="w-full mb-2 font-medium">Badges</h3>
                ...
                <TooltipTrigger asChild>
                  <div 
                    className={`w-8 h-8 flex items-center justify-center rounded-full text-lg
                      ${badge.color} bg-${badge.backgroundColor}/20 
                      border-2 border-${badge.backgroundColor}/60 cursor-pointer
